...
def train_gain(data_x, gain_parameters):
    ...
    debugging_file = "./debug/" + str(alpha) + "train_gain.log"
    logging.basicConfig(filename=debugging_file, level=logging.DEBUG)
    # define mask matrix
    ...
    optimD = torch.optim.Adam(netD.parameters(), lr=0.0001)
    optimG = torch.optim.Adam(netG.parameters(), lr=0.0001)
    ...
    for it in range(40000):
        ...
        # the next two lines is needed if D and G have the same input size
...
def main(args):
    ...
    norm_data, norm_parameters = data_loader(data_name, miss_rate, skip_rows, alpha, dir_name)
