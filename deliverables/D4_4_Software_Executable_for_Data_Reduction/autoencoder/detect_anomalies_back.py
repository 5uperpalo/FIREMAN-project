...
if __name__ == "__main__":
    ...
    for fault_no in faults_list:
        if fault_no in excluded_faults:
            continue
        if fault_no == 0:
            print(0, detect_anomalies(model, model_parameters, fault_no, loss_threshold))
        else:
            false_alarm_ratio.append(detect_anomalies(model, model_parameters, fault_no, loss_threshold))
